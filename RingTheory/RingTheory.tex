    ...
    \begin{enumerate}    
        ...
        \subsection{General Ring Theory}
            ...
            \begin{proof}
                \begin{enumerate}
                    ...
                    \item $-x_1a=-x_2a \Rightarrow -x_1ab=-x_2ab \Rightarrow x_1=x_2$
                    \item Suppose that $X$ is finite. Then $\phi$ is a bijection. So there exists some $x$ such that $b=b+1-xa \Rightarrow xa=1$. Then $x=xab=b$. Now take the other element in the set say $y\neq x$. Then $x=xay=y$. Which is a contradiction.
                \end{enumerate}
            \end{proof}
            ...
            \begin{proof}
                Let $\mathcal{M}$ be our set of matrices. Then define $\phi:\mathcal{M}\to \C$ by $\phi(M)=a+bi$. Then show that this is a ring isomorphism.
            \end{proof}
            ...
            \begin{proof}
                Let $\mathcal{M}$ be the set of matrices. Define $\phi:\mathcal{M}\to \Z[\sqrt{p}]$ by $\phi(M)=a+b\sqrt(P)$.
            \end{proof}
            ...
            \begin{proof}
                \begin{enumerate}
                    \item trivial
                    \item .
                    \item note that $2^2+3^2=13$
                \end{enumerate}
            \end{proof}

            %#29
            \item Let $I\subseteq J$ be right ideals of a ring $R$ such that $J/I \cong R$ as right $R$-modules. Prove that there exists a right ideal $K$ such that $I\cap K=(0)$ and $I+K=J$.
        ...
        \subsection{Prime, Maximal, and Primary Ideals}
            ...
            \begin{proof}
                ...
                For (b) note that $\Z[x]/(x) \cong \Z$ so $(x)$ is a prime ideal of $\Z[x]$ but $(x)$ is not maximal in $\Z[x]$ since $\Z$ is not a field.
            \end{proof}
            ...
            \begin{proof}
                Suppose that $I$ and $J$ are ideals of $R$ such that $I \cap J \subseteq R$ but $J \nsubseteq P$. Then there is some element $x\in J$ such that $x\notin P$. 
            ...
            \begin{proof}
                Map $\phi:R\to R/M_1\otimes R/M_2$ by $\phi(r)=(r+M_1)+(r+M_2)$. Note that $M_1$ and $M_2$ are comaximal ideals since they are distinct maximal ideals i.e. $M_1\subsetneq M_1+M_2\subseteq R$ and so $M_1+M_2=R$ since $M_1$ is maximal. So we can write any element $r\in R$ as a sum $m_1+m_2$ where $m_1\in M_1$ and $m_2\in M_2$. Thus we can also write our map as $\phi(r)=\phi(m_1+m_2)=(m_2+M_1)+(m_1+M_2)$. 
                
                Now we need to show that $\phi$ is a ring homomorphism. Let $x,y\in R$. Then $x=x_1+x_2$ and $y=y_1+y_2$ where $x_1,y_1\in M_1$ and $x_2,y_2\in M_2$. Then $\phi(xy)=(xy+M_1)+(xy+M_2)=(x+M_1)(y+M_1)+(x+M_2)(y+M_2)=$ The proof follows by showing that $\phi$ is a surjective homomorphism with kernel $I$. Then after applying the First Isomorphism Theorem, the result is proved.
            \end{proof}
            ...
            \begin{proof}
                Note that $(x)$ is an ideal of $R[x]$ and $R[x]/(x) \cong R$. Then (a) and (b) follow trivially.
            \end{proof}
        ...
        \subsection{Domains}
            ...
            \begin{proof}
                Note that $R$ is noetherian since it is a PID. Suppose that there are infinitely many divisors of $a$, $d_1, d_2, d_3, \hdots$, then we have the infinite ascending chain $(a)\subseteq (d_1d_2d_3\hdots) \subseteq (d_2d_3\hdots) \subseteq \hdots$. But this chain does not terminate, a contradiction to $R$ being Noetherian.
            \end{proof}
            ...
            \begin{proof}
                Suppose there is an $\alpha \in F$ such that $\alpha^2=d$. Then that polynomial $x^2-d$ is reducible in $F[x]$ but is not irreducible in $D$ since if it was there would exist some element $a\in D$ such that $a^2=d$, a contradiction to $d$ being irreducible. Finally we arrive at a contradiction to Gauss's Lemma since we have a polynomial in a UFD that is irreducible over $D$ but reducible in its field of fractions $F$.
            \end{proof}
            ...
            \begin{proof}
                Let $D$ be a Euclidean domain and $P$ be a non-zero prime ideal. Since $D$ is a Euclidean domain, then it is a PID. Thus we can say that $P=(p)$ for some $p\in D$. Note that $(p)$ is a prime ideal if and only if $p$ is a prime element. So we can say that $p$ is a prime element. Now suppose $P\subseteq M \subseteq (1) =D$. Then $M=(m)$ and so we have $(p)\subseteq (m) \subsetneq (1)$. But then $p\in (m)$ and so $p=rm$. Thus $p\mid r$ or $p\mid m$. Now if $p\mid r$, we have $r=ps$ or $p=rm=psm$ or $1=sm$ of $(m)=(1)=D$, a contradiction to $M$ being maximal. Therefore, we must have $p\mid m$ and so $m=pr$ and thus $(m)=(p)$. Thus $(p)=P$ is maximal. 
            \end{proof}
            ...
            \begin{proof}
                Suppose $\pi$ is irreducible. Then whenever $\pi=xy$ for $x,y\in R$ then $x$ is a unit or $y$ is a unit. Now suppose that $(\pi)\subseteq I\subseteq R$. Since $R$ is a PID, we can say there exists an element $x$ such that $I=(x)$. Then we have $(\pi)\subseteq (x) \subseteq R$. Now we can say there exists an element $y$ such that $\pi = xy$ and so $x$ is a unit or $y$ is a unit. If $x$ is a unit, then $(x)=R$. If $y$ is a unit, then $\pi\mid x$ and so $(x)\subseteq (\pi)$. Thus $(\pi)=(x)$. Since our choice of $I$ was arbitrary, we can conclude that $(\pi)$ is maximal and thus prime. Now if $rs\in (\pi)$ then $r\in (\pi)$ or $s\in (\pi)$. Thus $\pi a=rs$ and so $\pi b=r$ or $\pi c=s$. Therefore if $\pi \mid rs$ then $\pi \mid r$ or $\pi \mid s$, as desired.
            \end{proof}
